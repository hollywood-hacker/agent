...
def make_exe():
    ...
    python_config.run_command = "import sys; sys.path.append('.'); import worker; worker.main()"

    # Exclude markupsafe extension module from being added
    excluded_modules = ["markupsafe"]
    ...
    # Create a setup.py install command excluding the specified modules
    setup_command = exe.setup_py_install(package_path=CWD)

    # Filter out the excluded modules from the collected resources
    setup_command = [
        resource for resource in setup_command
        if resource.name not in excluded_modules
    ]

    # Add the collected resources to the embedded resources
    exe.add_python_resources(setup_command)
